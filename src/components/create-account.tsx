...
function toFieldErrors(errors: Array<unknown> | undefined): Array<{ message?: string }> {
  if (!errors?.length) {
    return []
  }
...
export default function CreateAccount() {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        const result = await clientSignUp(value)
        if (result.error) {
          setSubmitError(getSignUpErrorMessage(result.error))
        }
      } catch (error: unknown) {
      ...
      <div className='flex flex-1 flex-col justify-center px-4 py-10 lg:px-6'>
        <div className='sm:mx-auto sm:w-full sm:max-w-md'>
          <div className='flex items-center space-x-1.5'>
            <Logo className='h-7 w-7 text-foreground dark:text-foreground' aria-hidden={true} />
            <p className='text-pretty text-lg font-medium text-foreground dark:text-foreground'>Acme</p>
          </div>
          ...
          >
            ...
            >
              {(field) => (
                <Field data-invalid={field.state.meta.isTouched ? !field.state.meta.isValid : undefined}>
            ...
            <form.Subscribe
              ...
              children={([canSubmit, isSubmitting]) => (
                <Button type='submit' disabled={!canSubmit} className='mt-4 w-full py-2 font-medium'>
